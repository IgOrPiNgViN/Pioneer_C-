...
Современные стандарты C++ (C++11, C++14, C++17, C++20) добавили множество
новых возможностей, которые делают программирование более удобным и эффективным.

ЗАЧЕМ НУЖНЫ СОВРЕМЕННЫЕ ВОЗМОЖНОСТИ:

1. Упрощение кода: меньше кода, больше выразительности
2. Безопасность: автоматическое управление ресурсами
3. Производительность: оптимизация на этапе компиляции
4. Удобство: более интуитивный синтаксис

1. ЛЯМБДА-ВЫРАЖЕНИЯ (Lambda Expressions, C++11):

Лямбда-выражения - это анонимные функции, которые можно определять прямо в коде.
...
б) С захватом переменных:
   int multiplier = 5;
   auto multiply = [multiplier](int x) { return x * multiplier; };
   int result = multiply(10);  // result = 50

в) С захватом по ссылке:
   int counter = 0;
   auto increment = [&counter]() { counter++; };
   increment();  // counter = 1

г) С захватом всех переменных:
   [=]  // захват по значению
   [&]  // захват по ссылке
   [=, &var]  // все по значению, var по ссылке

Применение:
- Алгоритмы STL (find_if, count_if, transform)
- Callback функции
- Асинхронное программирование

2. АВТОМАТИЧЕСКОЕ ВЫВЕДЕНИЕ ТИПОВ (auto, C++11):

Ключевое слово auto позволяет компилятору автоматически определить тип переменной.

Синтаксис:
auto variable = value;

Примеры:
auto name = string("Иван");      // auto = string
auto age = 25;                   // auto = int
auto height = 1.75;              // auto = double
auto isStudent = true;            // auto = bool

auto vec = vector<int>();         // auto = vector<int>
auto it = vec.begin();            // auto = vector<int>::iterator

Преимущества:
- Меньше кода
- Удобство при работе с шаблонами
- Автоматическая адаптация к изменениям типов

Ограничения:
- Нельзя использовать для параметров функций (до C++14)
- Нельзя использовать для возвращаемого типа без явного указания

3. УМНЫЕ УКАЗАТЕЛИ (Smart Pointers, C++11):

Умные указатели автоматически управляют памятью, предотвращая утечки.

а) unique_ptr - единоличное владение:
   #include <memory>
...
б) shared_ptr - разделяемое владение:
   shared_ptr<int> ptr1 = make_shared<int>(42);
   shared_ptr<int> ptr2 = ptr1;  // оба указывают на один объект
   
   Особенности:
   - Несколько shared_ptr могут владеть одним объектом
   - Подсчет ссылок (reference counting)
   - Автоматическое освобождение, когда последний shared_ptr уничтожен

в) weak_ptr - слабая ссылка:
   weak_ptr<int> weak = ptr1;  // не увеличивает счетчик ссылок
   
   Особенности:
   - Не владеет объектом
   - Используется для разрыва циклических ссылок
   - Можно проверить, существует ли объект

4. RANGE-BASED FOR LOOPS (C++11):

Упрощенный синтаксис для перебора элементов контейнеров.

Синтаксис:
for (тип element : container) { ... }
...
в) С ссылкой (для изменения):
   for (auto& element : vec) {
       element *= 2;  // изменяем элементы
   }

г) С const ссылкой (для чтения):
...
- STL контейнерами
- Строками
- Пользовательскими типами (с begin/end)

5. INITIALIZER LISTS (C++11):

Унифицированная инициализация для различных типов.

Синтаксис:
тип variable = {значение1, значение2, ...};
...
Перемещение вместо копирования для повышения производительности.

Синтаксис:
тип&& rvalue_reference = move(variable);

Примеры:
string str1 = "Hello";
string str2 = move(str1);  // перемещение, str1 становится пустым

vector<int> vec1 = {1, 2, 3};
vector<int> vec2 = move(vec1);  // перемещение, vec1 становится пустым

Преимущества:
- Избежание дорогостоящего копирования
- Повышение производительности
- Эффективная работа с большими объектами
...
Синтаксис:
constexpr тип функция(параметры) { ... }
...
- Использование в константных выражениях
...
print(1, 2.5, "Hello", 'c');  // выводит: 1 2.5 Hello c

Применение:
- Функции с переменным количеством аргументов
- Универсальные обертки
- Реализация tuple, variant и т.д.

ДОПОЛНИТЕЛЬНЫЕ ВОЗМОЖНОСТИ C++14, C++17, C++20:

C++14:
- Обобщенные лямбды
- Возвращаемый тип auto для функций
- Переменные шаблоны
